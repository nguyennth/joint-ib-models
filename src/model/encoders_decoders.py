...
class BERTEncoder(nn.Module):
    """ 
    Wrap BERT as an encoder
    """
    def __init__(self, max_sent_length, device, pretrained_model='bert-base-uncased'):
        ...
        # self.net = torch.hub.load('huggingface/pytorch-transformers', 'model', 'bert-base-uncased', config=self.config) 
        self.net = BertModel.from_pretrained(pretrained_model)
        self.max_sent_length= max_sent_length

    def forward(self, tokens_tensor, input_mask):
        
        # tokens_tensor =  torch.tensor(indexed_source).to(self.device)     

        token_type_ids = torch.tensor(np.zeros(tokens_tensor.shape)).long().to(self.device)
    
        encoder_outputs = self.net(tokens_tensor, token_type_ids=token_type_ids, attention_mask=input_mask)
        
        # print(encoder_outputs.last_hidden_state.shape)
        # print(encoder_outputs.attentions.shape)
