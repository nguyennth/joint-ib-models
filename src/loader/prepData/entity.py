...
def process_entities(entities1, triggers1, sentences1, params, files_fold):
    ...
    for pmid in entities0:
        ...
        for xx, sentence in enumerate(sentences):
            ...
            for t1 in terms_sentence:
                ...
                eids.append(eid)
                # if eid in norm_id:
                #     norm_id[eid] = (norms[eid]['concept_id'], t1) 
            
            sentence['eids'] = eids
            # sentence['entity_norm'] = norm_id
            ...
            for x, id_ in enumerate(eids):  # for every entity if it belongs to sentence span
                ...
                if (span[0] <= b <= span[1]) and (span[0] <= e <= span[1]):
                    ...
                    print("{0} SKIP ENTITY: start: {1}, end: {2}".format(pmid, str(b), str(e)))
            ...
            for eid in eids:
                if "offs2" not in readable_ents[eid]:
                    print("The skipped entity: ", readable_ents[eid])
                    continue
...
def extract_entities(sw_sentence, tag2id_mapping, id2tag_mapping, nn_mapping):
    ...
    for idx, tag in enumerate(tags):
        ...
        merging_matrix = np.full((num_rows, num_cols), 0, dtype=np.int32)
        ...
        token_start_indices = np.zeros(merging_matrix.shape, dtype=np.int32)
        token_start_indices[
        ...
        token_end_indices = np.zeros(merging_matrix.shape, dtype=np.int32)
        token_end_indices[
        ...
        for begin_pos, end_pos in zip(
            ...
            if tag_id > 0:  # Tag O is not an entity
                ...
                if (begin_pos, end_pos) not in terms:
                    terms[(begin_pos, end_pos)].append(term_name)
                else: #to fix duplicate in MedMention
                    list_term = terms[(begin_pos, end_pos)] 
                    if term_name not in list_term:
                        terms[(begin_pos, end_pos)].append(term_name)
                    else:
                        print('Duplicate term: ' + str(begin_pos) + ':' + str(end_pos) + ':' + term_name)
                
                if (begin_pos, end_pos) not in entities:
                    entities[(begin_pos, end_pos)].append(nn_mapping['tag_id_mapping'][tag_name])
                    if len(entities[(begin_pos, end_pos)]) == 0:
                        print("debug")
                else:  #to fix duplicate in MedMention
                    list_ents = entities[(begin_pos, end_pos)]
                    if nn_mapping['tag_id_mapping'][tag_name] not in list_ents:
                        entities[(begin_pos, end_pos)].append(nn_mapping['tag_id_mapping'][tag_name])
                    else:
                        print('Duplicate term: ', tag_name)
                

    return entities, terms, sw_sentence


def convert_to_sub_words(word_tokens, tags, tags_terms, tokenizer_encoder=None):
    subword_pos = 0
    ...
    for token_idx, token in enumerate(word_tokens):
        if tokenizer_encoder:
            subtokens = tokenizer_encoder.tokenize(token)
            if subtokens:
